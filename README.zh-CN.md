...
Chrome 没有开放「已保存标签组」的扩展 API，所以这个 workflow 自己维护一份列表。在任意标签组上按 `⌘⌥↩` 即可保存它的名称、颜色和标签页网址。已保存的组会带 `Saved` 标记出现在 `c2` 和 `c0` 里，排在当前打开的组之后。

在已保存组上回车会重新打开这些标签页并重新分组，`⌘↩` 删除这条保存记录。用同样的名称和颜色再保存一次，会更新原有记录而不是新增一条。

保存始终是显式操作。这个 workflow 不会通过比对前后两次读取来推断「某个组被关掉了」，所以改名或改颜色不会留下重复的残留条目。
